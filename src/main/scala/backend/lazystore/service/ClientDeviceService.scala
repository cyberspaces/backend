...
//
//  object GetClientDeviceAppVersionService extends GetClientDeviceAppVersionService with BaseAopService
//  private[service] class GetClientDeviceAppVersionService extends BaseService{
//    override def apply(request: RestRequest): ResponseContent = ???
//  }

  object GetClientDeviceAppsVersionService extends GetClientDeviceAppsVersionService with BaseAopService
  private[service] class GetClientDeviceAppsVersionService extends BaseService{
    override def apply(request: RestRequest): ResponseContent = {
      val titles=Parser.JsonStringToList(Parser.ChannelBufferToString(request.underlying.getContent))
      val content=ClientDeviceDao.getClientDeviceVersions(titles)
      ResponseContent(content)
    }
  }
