...
//
//object DeviceAppVersionAction extends RestAction[RestRequest,ResponseContent]{
//  override def apply(request: RestRequest): ResponseContent = ???
//}

object DeviceAppVersionsAction extends RestAction[RestRequest,ResponseContent]{
  override def apply(request: RestRequest): ResponseContent = ClientDeviceService.GetClientDeviceAppsVersionService(request)
}