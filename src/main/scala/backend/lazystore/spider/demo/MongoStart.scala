...
import backend.lazystore.spider.wandoujia.WandoujiaNosqlDataTransferMysqlJob
...
object MongoStart extends App{
  ...
  def updateCurrentAppVersion(res:String): Unit = {
...
//    }catch{
//      case e=>println(e.getLocalizedMessage+","+e.getMessage+","+e.getStackTrace)
//    }finally {
//      mongo.close()
//    }
  ...
  def insertDataToCollection(res:String): Unit = {
    ...
    dbObjs.map{it=>
      try {
        WandoujiaNosqlDataTransferMysqlJob.updateAPPVersion(it)
      }catch {
        case ex:Throwable=>ErrorLog(it.toString)
      }
    }
//    val mongo=new MongoClient("localhost",27017)
//
