...
object Tables {
  ...
  case class ApptagsRow(lazyappId: Long,lazyappTitle:String ,tag: String, weight: Int)
  /** GetResult implicit for fetching ApptagsRow objects using plain SQL queries */
  implicit def GetResultApptagsRow(implicit e0: GR[Long], e1: GR[String], e3:GR[String],e4: GR[Int]): GR[ApptagsRow] = GR{
    prs => import prs._
    ApptagsRow.tupled((<<[Long], <<[String],<<[String] ,<<[Int]))
  }
  ...
  class Apptags(_tableTag: Tag) extends Table[ApptagsRow](_tableTag, "apptags") {
    def * = (lazyappId,lazyappTitle,tag, weight) <> (ApptagsRow.tupled, ApptagsRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (lazyappId.?, lazyappTitle.?,tag.?, weight.?).shaped.<>({r=>import r._; _1.map(_=> ApptagsRow.tupled((_1.get, _2.get, _3.get,_4.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    
    /** Database column lazyapp_id DBType(BIGINT UNSIGNED), AutoInc */
    val lazyappId: Column[Long] = column[Long]("lazyapp_id")
    val lazyappTitle:Column[String]=column[String]("lazyapp_title",O.Length(25,varying=true))
    /** Database column tag DBType(VARCHAR), Length(25,true) */
    val tag: Column[String] = column[String]("tag", O.Length(60,varying=true))
  ...
  case class LazyadminRow(var id: Long,var  role: String,var  name: String,var  passwd: String,var email: String)
  ...
  case class UDeviceRow(var id: Long, var name: String, position: String, var registerdate: Long, var uuid: String, system: String, producer: String, var isbind: Byte,var phone: String, providername: String)
